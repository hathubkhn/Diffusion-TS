...
datasets = {
    "nn5_daily": (9, "nn5_daily.tsf", None, False, "strict"),
    ...
    "fred_md": (15, "fred_md.tsf", 12, False, "strict"),
    ...
    "solar_weekly": (6, "solar_weekly.tsf", 5, False, "strict"),
    ...
    "rideshare": (168, "rideshare_dataset_without_missing_values.tsf", 168, False, "overlap"),
    # lag used to be 210, was too long to create single train example
    "temperature_rain": (9, "temperature_rain.tsf", 30, False, "strict"),
}
                    ...
                    if len(col_names) == 0:
                        ...
                        for i in range(len(col_names)):
                            ...
                            if col_types[i] == "numeric":
                                ...
                                raise Exception(
                                    "Invalid attribute type.")  # Currently, the code supports only numeric, string and date types. Extend this as required.
...
class _MonashDataset(Dataset):
    ...
    def __init__(
        ...
        self.train_series_times = [pd.date_range(start=start_time, periods=len(series), freq=self.freq) for
                                   start_time, series in zip(self.train_start_times, self.train_series_list)]
        self.full_series_times = [pd.date_range(start=start_time, periods=len(series), freq=self.freq) for
                                  start_time, series in zip(self.train_start_times, self.full_series_list)]
        ...
        if save_processed and os.path.exists(os.path.join(data_dir, processed_filename)):
            self.data, self.data_times, self.target, self.target_times, self.ts_indices = np.load(
                os.path.join(data_dir, processed_filename), allow_pickle=True)
        else:
            if self.split != 'test':
                ...
                self.data = [ts[-self.forecast_horizon - self.min_lag:-self.forecast_horizon] for ts in
                             self.full_series_list]
                self.data_times = [ts[-self.forecast_horizon - self.min_lag:-self.forecast_horizon] for ts in
                                   self.full_series_times]
            ...
            if save_processed:
                ...
                np.save(os.path.join(data_dir, processed_filename),
                        (self.data, self.data_times, self.target, self.target_times, self.ts_indices))
    ...
    def __getitem__(self, idx):
        ...
        context = self.data[loc][
                  max(0, idx - self._cume[loc] - (self.lag - self.min_lag)): idx - self._cume[loc] + self.min_lag]
        context_time = self.data_times[loc][
                       max(0, idx - self._cume[loc] - (self.lag - self.min_lag)): idx - self._cume[loc] + self.min_lag]
        ...
        # To tensors
        context, target = torch.tensor(context, dtype=torch.float).unsqueeze(1), torch.tensor(target,
        ...
        if not self.autoregressive:
            ...
            context = torch.cat([torch.zeros(self.lag - actual_lag, 1), context, torch.zeros(self.forecast_horizon, 1)],
                                dim=0)
    ...
    def _timestamp_to_features(self, timestamp, pad=0):
        if pad == 0:
            ...
            return {k: torch.cat([torch.zeros(pad, dtype=torch.long) - 1,
                                  torch.tensor(np.array(getattr(timestamp, k)), dtype=torch.long) + 1]) for k in
                    self.TIME_FEATURES}
