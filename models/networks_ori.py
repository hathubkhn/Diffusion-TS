...
class AttentionOp(torch.autograd.Function):
    ...
    #     # Giả sử multi-head, reshape w về [B, num_heads, Q, K]
    #     num_heads = 1  # điều chỉnh theo model của bạn
    #     w_reshaped = w.view(B, num_heads, Q//num_heads, K)
        
    #     for h in range(min(num_heads, 4)):  # show một vài head
    #         attn = w_reshaped[batch_idx, h].cpu().detach().numpy()
