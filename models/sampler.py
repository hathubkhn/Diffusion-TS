...
class DiffusionProcess():
    ...
    def impute(self, x, latents, mask, ref_hist,ref_future, top_k,num_heads, class_labels=None):
        ...
        for i, (t_cur, t_next) in enumerate(zip(t_steps[:-1], t_steps[1:])):  # 0, ..., N-1
            ...
            denoised = self.net(x_hat, t_hat, ref_hist = ref_hist,ref_future=ref_future, top_k = top_k, num_heads = num_heads, class_labels = class_labels).to(torch.float64)
            ...
            if i < self.num_steps - 1:
                ...
                denoised = self.net(x_next, t_next, ref_hist = ref_hist,ref_future=ref_future, top_k = top_k,num_heads = num_heads,class_labels = class_labels).to(torch.float64)
    ...
    def interpolate(self, x, mask, ref_hist,ref_future, xT=None):
        ...
        return self.impute(x, xT, mask, ref_hist = ref_hist,ref_future=ref_future, top_k=self.args.top_k, num_heads=self.args.num_cross)
