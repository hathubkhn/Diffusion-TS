...
class Conv2d(torch.nn.Module):
    ...
    def forward(self, x, up=False, ref=None):
        ...
        # if ref is not None:
        #     w_ref = self.weight.to(ref.dtype) if self.weight is not None else None
        #     b_ref = self.bias.to(ref.dtype) if self.bias is not None else None
        #     f_ref = self.resample_filter.to(ref.dtype) if self.resample_filter is not None else None

        #     if self.fused_resample and self.up and w is not None:
        #         ref = torch.nn.functional.conv_transpose2d(ref, f.mul(4).tile([self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=max(f_pad - w_pad, 0))
        #         ref = torch.nn.functional.conv2d(ref, w, padding=max(w_pad - f_pad, 0))
        #     elif self.fused_resample and self.down and w is not None:
        #         ref = torch.nn.functional.conv2d(ref, w, padding=w_pad+f_pad)
        #         ref = torch.nn.functional.conv2d(ref, f.tile([self.out_channels, 1, 1, 1]), groups=self.out_channels, stride=2)
        #     else:
        #         if self.up:
        #             ref = torch.nn.functional.conv_transpose2d(ref, f.mul(4).tile([self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad)
        #         if self.down:
        #             ref = torch.nn.functional.conv2d(ref, f.tile([self.in_channels, 1, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad)
        #         if w is not None:
        #             ref = torch.nn.functional.conv2d(ref, w, padding=w_pad)
        #     if b is not None:
        #         x = x.add_(b.reshape(1, -1, 1, 1))
        return x
...
class UNetBlock(torch.nn.Module):
    ...
    def forward(self, x, emb, up=False, ref=None):
        ...
        if self.num_heads:
            ...
            x = x * self.skip_scale
            # shape of x
            #print(f"Shape Unet of x: {x.shape}")
...
class CrossAttentionBlock(nn.Module):
    def __init__(self, in_channels, out_channels, num_heads, top_k, model, preprocess):
        super().__init__()
        
        self.top_k = top_k
        
        # Model CLIP embedding
        ...
        self.q_proj = nn.Conv2d(in_channels, 3, kernel_size=1)
        # self.k_proj = nn.Conv2d( out_channels, 3, kernel_size=1)
    ...
    def to_clip_embedding(self, batch_tensor):
        
        
        
        batch_tensor = self.q_proj(batch_tensor)  # (B, 3, H, W)
        B, C, H, W = batch_tensor.shape
        embeddings = []


        #img = transforms.ToPILImage()(batch_tensor[i].cpu())
        img = batch_tensor
        img = self.transform(img).to('cuda')  # (B *C, 256, 256)

        #img = self.preprocess(img).unsqueeze(0).to('cuda')  # (1, 3, 256, 256)
        with torch.no_grad():
            feat = self.model_emb.encode_image(img)  # (1, 512)
            
            
        embeddings.append(feat)

        return torch.cat(embeddings, dim=0)  # (B, 512)
    ...
    def compute_similarity(self, query, key):
        ...
        combined_emb = self.to_clip_embedding(combined)  # (B * (top_k + 1), 512)
        combined_emb = combined_emb.view(B, T, -1)  # (B, top_k + 1, 512)
        query_emb = combined_emb[:, 0, :]           # (B, 512)
        key_emb = combined_emb[:, 1:, :]            # (B, top_k, 512)
        # Normalize
        query_emb = F.normalize(query_emb, dim=1)     # (B, 512)
        key_emb = F.normalize(key_emb, dim=2)         # (B, top_k, 512)
    ...
    def forward(self, x, ref, top_k, block, block_ref, emb = None):
        for name, param in self.model_emb.named_parameters():
            if param.requires_grad:
                print("⚠️ Still trainable:", name)

        B, C, H, W = x.shape
        # Query
        q = x  # (B, C, H, W)
        ref_list = list(torch.chunk(ref, chunks=self.top_k, dim=1))   # list of (B, H, W)
        ref_cat = torch.cat(ref_list, dim=0)  # (B * top_k, H, W)
        kv = []
           
        
        # if isinstance(block_ref, UNetBlock):
        #     r_out_cat = block_ref(ref_cat, emb=None) 
        # else:
        #     r_out_cat = block_ref(ref_cat)    
        # 
        if isinstance(block_ref, UNetBlock):
            r_out_cat = block_ref(ref_cat, emb=None) 
        else:
        ...
        B = ref_list[0].shape[0]
        kv = list(torch.chunk(r_out_cat, chunks=self.top_k, dim=0))
             
        k = kv
        v = [t.clone().detach() for t in kv]  
        ...
        return output, torch.cat(kv, dim=1)
#----------------------------------------------------------------------------
...
class DhariwalUNet(torch.nn.Module):
    def __init__(self,
        ...
        #Model embedding
        device = "cuda" if torch.cuda.is_available() else "cpu"
        self.model_emb, _, self.preprocess = open_clip.create_model_and_transforms(
            'ViT-B-32', pretrained='laion2b_s34b_b79k', device=device
        )
        ...
        for level, mult in enumerate(channel_mult):
            ...
            if level == 0:
                ...
                self.enc_cross_attn[f'{res}x{res}_conv'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
            
            else:
                self.enc[f'{res}x{res}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                self.enc_ref[f'{res}x{res}_down'] = UNetBlock(in_channels=cout, out_channels=cout, down=True, **block_kwargs)
                self.enc_cross_attn[f'{res}x{res}_down'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
            ...
            for idx in range(num_blocks):
                ...
                self.enc_cross_attn[f'{res}x{res}_block{idx}'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
        ...
        for level, mult in reversed(list(enumerate(channel_mult))):
            ...
            if level == len(channel_mult) - 1:
                ...
                self.dec_cross_attn[f'{res}x{res}_in0'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
                
                self.dec[f'{res}x{res}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, **block_kwargs)
                self.dec_ref[f'{res}x{res}_in1'] = UNetBlock(in_channels=cout, out_channels=cout, **block_kwargs)
                self.dec_cross_attn[f'{res}x{res}_in1'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
                
            else:
                self.dec[f'{res}x{res}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
                self.dec_ref[f'{res}x{res}_up'] = UNetBlock(in_channels=cout, out_channels=cout, up=True, **block_kwargs)
                self.dec_cross_attn[f'{res}x{res}_up'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
            ...
            for idx in range(num_blocks + 1):
                ...
                self.dec_cross_attn[f'{res}x{res}_block{idx}'] = CrossAttentionBlock(in_channels=cout, out_channels=cout, num_heads=4, top_k = self.top_k, model=self.model_emb, preprocess=self.preprocess)
    ...
    def forward(self, x, ref, top_k, noise_labels, class_labels, augment_labels=None):
        ...
        for key in self.enc.keys():
            ...
            x_skip = x.clone()
            #print("Encoder block: ", key)
            #print(f"Shape of x, ref before attention: {x.shape}, {ref.shape if ref is not None else 'None'}")
            # print(f"Shape of x in CrossAttentionBlock: {x.shape}")  # (B, C, H, W)
            # print(f"Shape of ref in CrossAttentionBlock: {ref.shape}")
        ...
        # print(f"Shape of x after U: {x.shape}")    
...
class EDMPrecond(torch.nn.Module):
    ...
    def forward(self, x, sigma, ref,top_k, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        #print(f"Shape of x EDM: {x.shape}")
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
