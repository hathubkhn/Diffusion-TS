...
def main():
    ...
    for convert_method in args.convert_method:
        ...
        # Initialize dictionaries to store per-symbol lists
        query_image_tensors_dict = {}
        reference_image_tensors_dict = {}
        query_date_keys_dict = {}
        reference_date_keys_dict = {}
        reference_symbols_dict = {}

        for symbol in all_query_symbol:
            query_image_tensors_dict[symbol] = []
            reference_image_tensors_dict[symbol] = []
            query_date_keys_dict[symbol] = []
            reference_date_keys_dict[symbol] = []
            reference_symbols_dict[symbol] = []
        ...
        for symbol in tqdm(df['Symbol'].unique(), desc="Xử lý symbol"): #df['Symbol'].unique()
            ...
            if symbol not in all_query_symbol:
                for i, date_key in enumerate(date_keys):
                  for query_symbol in all_query_symbol:
                      
                    reference_image_tensors_dict[query_symbol].append(embeddings[i])
                    reference_date_keys_dict[query_symbol].append(date_key)
                    reference_symbols_dict[query_symbol].append(symbol)
              ...
              for query_symbol in all_query_symbol:
                if symbol == query_symbol:
                    for i, date_key in enumerate(date_keys):
                        # if symbol == query_symbol:                       
                        query_image_tensors_dict[query_symbol].append(embeddings[i])
                        query_date_keys_dict[query_symbol].append(date_key)
        ...
        for query_symbol in all_query_symbol:
            query_image_tensors = query_image_tensors_dict[query_symbol]
            reference_image_tensors = reference_image_tensors_dict[query_symbol]
            query_date_keys = query_date_keys_dict[query_symbol]
            reference_date_keys = reference_date_keys_dict[query_symbol]
            reference_symbols = reference_symbols_dict[query_symbol]

            if not query_image_tensors or not reference_image_tensors:
                print(f"Bỏ qua {query_symbol}, không đủ ảnh")
                continue
