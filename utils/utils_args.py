...
def parse_args_cond():
    ...
    parser.add_argument('--delay', type=int,
                        help='delay for the delay embedding transformation, only needed if using delay embedding') #1
    parser.add_argument('--embedding', type=int,
                        help='embedding for the delay embedding transformation, only needed if using delay embedding')  #2

    # --- model---
    parser.add_argument('--img_resolution', type=int, help='image resolution') #3
    parser.add_argument('--input_channels', type=int,
                        help='number of image channels, 2 if stft is used, 1 for delay embedding') 
    parser.add_argument('--unet_channels', type=int, help='number of unet channels')  #4
    parser.add_argument('--ch_mult', type=int, help='ch mut', nargs='+') #5 
    parser.add_argument('--channel_mult_emb', type=int, help='channel mult emb', nargs='+') #5 
    parser.add_argument('--num_blocks', type=int, help='number of blocks in the unet')
    parser.add_argument('--attn_resolution', type=int, help='attn_resolution', nargs='+') #6
    parser.add_argument('--diffusion_steps', type=int, help='number of diffusion steps') #7
